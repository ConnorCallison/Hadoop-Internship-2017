...
origDatabase=oracle_imports
resultDatabase=default
baseDir=$HADOOP_HOME/tmp/validation/${origDatabase}_${resultDatabase}
...
for bucket in $listOfTables ; do
    ...
    # Add an '&' after "( hive $queueConf -f $hiveExecuteFile_orig &> ${hiveExecuteFile_orig}.log )" to run the queries in parellel. (Had to disable on single node proof of concept)
