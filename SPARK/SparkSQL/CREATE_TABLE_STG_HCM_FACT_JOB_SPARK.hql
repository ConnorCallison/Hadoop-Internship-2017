...
-- Create STG_HCM_FACT_JOB_SPARK Table
-- Author: Connor Callison (ccallison@humboldt.edu)
-- Date Last Modified: 2/28/2017

DROP TABLE IF EXISTS STG_HCM_FACT_JOB_SPARK;
DROP TABLE IF EXISTS TEMP_STG_HCM_FACT_JOB_SPARK_1;
DROP TABLE IF EXISTS STG_HCM_FACT_JOB_RANKS;

-- Create smaller rank table
CREATE TABLE STG_HCM_FACT_JOB_RANKS
...
INSERT INTO STG_HCM_FACT_JOB_RANKS 
SELECT
...
where c.emplid not like 'DUP%'
order by C.EMPLID , C.EMPL_RCD , C.EFFDT , C.EFFSEQ;   

CREATE TABLE STG_HCM_FACT_JOB_SPARK (
  `JOB_DAYSEQ_KEY`        DECIMAL(38,0)     
...
INSERT INTO STG_HCM_FACT_JOB_SPARK
SELECT
...
,'-'
, 'HHUMPRD'
,  CASE WHEN D_DAT.FISCAL_YEAR < 2005 THEN 2005 ELSE D_DAT.FISCAL_YEAR END
...
FROM 
 PS_JOB C
 JOIN PS_CSU_JOB Z ON Z.EMPLID = C.EMPLID  AND Z.EMPL_RCD = C.EMPL_RCD AND Z.EFFDT = C.EFFDT AND Z.EFFSEQ = C.EFFSEQ
 JOIN STG_HCM_FACT_JOB_RANKS R ON R.JOB_DAYSEQ_KEY = CAST(CONCAT(date_format(c.effdt,'yyyyMMdd'), lpad(cast(cast(c.effseq as Int) as string), 4, '0'), C.EMPLID, lpad(cast(cast(c.EMPL_RCD as Int) as string), 4, '0')) as DECIMAL(38,0))
 ...
 LEFT JOIN STG_HCM_PRE_PER_ORG PO ON PO.EMPLID = C.EMPLID AND PO.EMPL_RCD = C.EMPL_RCD
...
WHERE 
  c.emplid not like 'DUP%'
  AND
  PO.EMPL_RCD = C.EMPL_RCD
  AND
  R.dw_jobseq not in 
    (select min(a.dw_jobseq)
     from STG_HCM_FACT_JOB_RANKS a)
